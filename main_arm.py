...
class Visualizer:
    ...
    def set_kinematics_panel(self):
        ...
        # self.listener.start() # REMOVE WHEN DONE

        # vX button
        self.x_neg = ttk.Button(
            self.control_frame, text="vx-", command=self.deactivate_VK
        )
        self.x_neg.grid(column=0, row=row_number, columnspan=1, pady=2)

        self.x_pos = ttk.Button(
            self.control_frame, text="vx+", command=self.deactivate_VK
        )
        self.x_pos.grid(column=1, row=row_number, columnspan=1, pady=2)
        row_number += 1
        # vY button
        self.x_neg = ttk.Button(
            self.control_frame, text="vy-", command=self.deactivate_VK
        )
        self.x_neg.grid(column=0, row=row_number, columnspan=1, pady=2)
        ...
        # vZ button
        self.x_neg = ttk.Button(
            self.control_frame, text="vz-", command=self.deactivate_VK
        )
        self.x_neg.grid(column=0, row=row_number, columnspan=1, pady=2)
