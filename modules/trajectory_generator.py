...
class QuinticPolynomial:
    ...
    def _copy_params(self, trajgen):
        ...
        self.final_acc = trajgen.start_acc
        self.T = trajgen.T
        self.ndof = trajgen.ndof
        self.X = [None] * self.ndof
    ...
    def solve(self):
        """
        solve the quintic path planning problem for the specified number of joints
        and movement range."""
        t0, tf = 0, self.T
        self.A = np.array(
            [
...
class TrapezoidVelocity:
    ...
    def generate(self, nsteps=100):
        proprotion_accel = 0.2  # percent
        self.t = np.linspace(0, self.T, nsteps)
        for i in range(self.ndof):  # iterate through all DOFs
            q, qd, qdd = [], [], []

            V = 1.5 * ((self.final_pos[i] - self.start_pos[i]) / self.T)
            tb = (self.start_pos[i] - self.final_pos[i] + V * self.T) / V
            alpha = V / tb

            for t in self.t:  # iterate through time, t
                if t < tb:
                    q.append(self.start_pos[i] + (alpha * t**2) / 2)
                    qd.append(alpha * t)
                    qdd.append(alpha)
                elif t < self.T - tb:
                    q.append(
                        (self.final_pos[i] + self.start_pos[i] - V * self.T) / 2 + V * t
                    )
                    qd.append(V)
                    qdd.append(0)
                else:
                    ...
                    qd.append(alpha * self.T - alpha * t)
                    qdd.append(-alpha)
